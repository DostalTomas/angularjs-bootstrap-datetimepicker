...
describe('minute view with initial date of 2013-01-22 1:15', () => {
    ...
    it('has `.switch` element with a value of 2013-Jan-22 1:00', () => {
    ...
    it('changes date/time to 1:00 to when clicking first `.minute` element with a value of 0:00', () => {
        expect(jQuery('.active', element).text()).toBe('1:15 AM');
...
describe('view returns to startView after setting time', () => {
    let rootScope;
    let element;
    beforeEach(() => angular.mock.module(dateTimePickerModule));
    beforeEach(() => inject(($compile, $rootScope) => {
    ...
    it('has `.switch` element with a value of Jan 22, 2013', () => {
        expect(jQuery('.switch', element).text()).toBe('Jan 22, 2013');
    });
    it('changes date/time to 1:00 to when clicking first `.hour` element with a value of 0:00 then returns to hour view', () => {
        expect(jQuery('.active', element).text()).toBe('1:00 AM');

        jQuery(jQuery('.hour', element)[0]).trigger('click');
        jQuery(jQuery('.minute', element)[0]).trigger('click');

        expect(rootScope.date).toEqual(DateTime.fromISO('2013-01-22T00:00:00.000').toJSDate());
